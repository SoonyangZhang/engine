...
void mock_file_handler(egn_t *egn,egn_event_t *ev){
    mock_file_conn_t *conn=(mock_file_conn_t*)ev->data;
    int fd,n=0;
    fd=conn->parent.fd;
    ...
    if(!ev->shut){
        if(conn->parent.repoints&EGNPOINTIN){
            while(true){
                ...
                if(n>0){
                    conn->recv_bytes+=n;
                    if(0==conn->total_bytes){
                        CHECK(n>=4);
                        int *p=(int*)buffer;
                        int x=(*p);
                        x=egn_ntoh32(x);
                        conn->total_bytes=x;
                        log_info("%d total bytes: %d",fd,x);
                    }
                    if(conn->total_bytes>0&&conn->recv_bytes>=conn->total_bytes){
                        fin=true;
                        break;
                    }
            ...
            if(fin){
                log_info("close %d recv bytes %d",fd,conn->recv_bytes);
...
inline mock_file_conn_t *create_tcp_conn(egn_t *egn,int fd,egn_event_handler handler){
    mock_file_conn_t *conn=nullptr;
    egn_event_t *ev=nullptr;
    int sz1=sizeof(mock_file_conn_t);
...
mock_file_conn_t* create_tcp_peer_conn(egn_t *egn,int fd,egn_event_handler handler){
    mock_file_conn_t *conn=create_tcp_conn(egn,fd,handler);
    egn_event_t *ev=nullptr;
    int points=0;
    if(!conn){
        close(fd);
...
void tcp_listen_handler(egn_t *egn,egn_event_t *ev){
    mock_file_conn_t *listen_conn=(mock_file_conn_t*)ev->data;
    ...
    if(!ev->shut){
        if(listen_conn->parent.repoints&EGNPOINTIN){
            while((s=accept(listen_fd,(sockaddr*)&sock_saddr,&sock_addr_sz))>=0){
                ...
                create_tcp_peer_conn(egn,s,mock_file_handler); 
                memset(ip_str,0,INET6_ADDRSTRLEN);
            }
        }
    }else{
...
mock_file_conn_t* create_tcp_listener(egn_t *egn,int family,
